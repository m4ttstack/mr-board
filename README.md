...
- **`GITLAB_TOKEN`** -- env var (bun auto-loads `.env`). needs `read_api` scope only; the board never writes to gitlab. without an env var, it falls back to the rt daemon's token-gated `secrets:read` (board scope) -- `rt secrets set board gitlabToken` on the machine running rt.
- **`SLACK_TOKEN`** -- optional user token (`xoxp-…`) for the review-thread integration. minted via the OAuth flow in `bun run setup`, or paste manually into `.env`. same rt-daemon fallback as `GITLAB_TOKEN` (as `slackToken`). without it, the Slack menu actions stay disabled and the board runs fine.
